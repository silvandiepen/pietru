...
export function render(
  ...
  const iconHtml = content.icon
    ? `<tr><td style="padding:0 0 12px 0;text-align:center"><img src="${esc(content.icon)}" alt="" width="48" height="48" style="display:inline-block;border:0" /></td></tr>`
    : ''

  const titleHtml = `<tr><td style="padding:0 0 12px 0"><h2 style="margin:0;font-family:${theme.headingFont};font-size:20px;line-height:28px;color:${theme.primaryColor};text-align:center">${esc(v(content.title, variables))}</h2></td></tr>`
  ...
  const ctaHtml = content.cta
    ? `<tr><td style="padding:12px 0 24px 0;text-align:center"><a href="${esc(v(content.cta.url, variables))}" style="display:inline-block;padding:12px 28px;background-color:${theme.primaryColor};color:#ffffff;font-family:${theme.fontFamily};font-size:15px;font-weight:600;text-decoration:none;border-radius:6px">${esc(v(content.cta.text, variables))}</a></td></tr>`
    : ''

  const html = shell(logoHtml, iconHtml, titleHtml, bodyHtml, ctaHtml, theme)
...
function buildText(
  ...
  if (content.cta) parts.push(`${v(content.cta.text, variables)}: ${v(content.cta.url, variables)}`)
