...
export function render(
  ...
  const titleHtml = `<tr><td style="padding:0 0 16px 0"><h1 style="margin:0;font-family:${theme.headingFont};font-size:24px;line-height:32px;color:${theme.primaryColor};text-align:center">${esc(v(content.title, variables))}</h1></td></tr>`

  const bodyHtml = `<tr><td style="padding:0 0 24px 0;font-family:${theme.fontFamily};font-size:16px;line-height:24px;color:${theme.textColor};text-align:center">${v(content.body, variables)}</td></tr>`

  const buttonHtml = `<tr><td style="padding:0 0 24px 0;text-align:center"><a href="${esc(v(content.buttonUrl, variables))}" style="display:inline-block;padding:16px 40px;background-color:${theme.primaryColor};color:#ffffff;font-family:${theme.fontFamily};font-size:18px;font-weight:600;text-decoration:none;border-radius:6px">${esc(v(content.buttonText, variables))}</a></td></tr>`

  const html = shell(logoHtml, titleHtml, bodyHtml, buttonHtml, theme)
...
function buildText(
  ...
  const parts: string[] = []
  parts.push(v(content.title, variables))
  parts.push(stripHtml(v(content.body, variables)))
  parts.push(`${v(content.buttonText, variables)}: ${v(content.buttonUrl, variables)}`)
