...
export function render(
  ...
  const greetingHtml = `<tr><td style="padding:0 0 8px 0"><h2 style="margin:0;font-family:${theme.headingFont};font-size:24px;line-height:32px;color:${theme.primaryColor}">${esc(v(content.greeting, variables))}</h2></td></tr>`
  ...
  const ctaHtml = content.cta
    ? `<tr><td style="padding:12px 0 24px 0;text-align:center"><a href="${esc(v(content.cta.url, variables))}" style="display:inline-block;padding:14px 32px;background-color:${theme.primaryColor};color:#ffffff;font-family:${theme.fontFamily};font-size:16px;font-weight:600;text-decoration:none;border-radius:6px">${esc(v(content.cta.text, variables))}</a></td></tr>`
    : ''

  const html = shell(logoHtml, greetingHtml, bodyHtml, ctaHtml, theme)
...
function buildText(
  ...
  const parts: string[] = []
  parts.push(v(content.greeting, variables))
